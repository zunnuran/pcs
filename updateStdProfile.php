...
<button type="button"class="close" style="position:relative; top:10px; right:10px;";" onclick=" document.getElementById('container').style.display='none'" ><a href="updateStudentProfile.php">&times;</a></button>
<h4 class="text-left">Update Profile</h4>
</div>
<div class="panel-body">
<form class="form-horizontal" action="updateStdProfile.php" method="post">
...
if (isset($_POST['submit'])) {
    ...
    if ($username == null) {
        ...
        echo "<script>alert('Please enter accesscode')</script>";
...
$query = "UPDATE userdetails SET username= '$username', fullname='$fullname', email='$email', contact='$contact', accesscode='$accesscode', registDate='$date' WHERE user_id='$userid' && userType=2";
        $result = mysqli_query($conn, $query);

        if ($result) {
            echo "<script>alert('Update Successfull !')</script>";
			echo "<script>window.open('viewStudentProfile.php', '_self')</script>";
        }
    }
}
?>
